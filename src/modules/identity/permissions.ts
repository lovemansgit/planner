...
const PERMISSIONS_DRAFT = {
  ...
  // ---- task --------------------------------------------------------------
  // Day-19 amendment to Day-5 lock — `task:create` is now user-flow per
  // memory/decision_task_module_amendment_v1.md §3. The single `createTask`
  // service fn dispatches on actor.kind: system actors bypass via
  // assertSystemActor (cron + bulk import + migration paths unchanged);
  // user actors require `task:create` (operator ad-hoc creation per Love's
  // Phase 1 vision — single-day or short-range one-off deliveries that
  // don't fit a subscription cadence). `task:delete` is still NOT in the
  // catalogue — operators cancel via `task:cancel` (sets internal_status
  // to CANCELED + pushes cancel to SF via outbound adapter).
  "task:create": {
    id: "task:create",
    resource: "task",
    action: "create",
    description:
      "Create a single ad-hoc task. Day-19 / Phase 1 amendment to Day-5 lock — operators can create one-off tasks for deliveries that don't fit a subscription cadence (catering orders, sample drops, replacements). System-actor branch (cron + bulk import) bypasses this perm via assertSystemActor; user-actor branch requires this perm.",
    systemOnly: false,
  },
  "task:read": {
    id: "task:read",
  ...
  // Day-19 / Phase 1 — single-task cancel. Operator transitions a future
  // task to internal_status='CANCELED' + outbound SF push to cancel the
  // SF-side task. Distinct from `subscription:skip` (which transitions a
  // task to SKIPPED with a compensating-date insert).
  "task:cancel": {
    id: "task:cancel",
    resource: "task",
    action: "cancel",
    description:
      "Cancel a single future task. Sets internal_status to CANCELED and pushes the cancel to SuiteFleet. Subject to the 18:00 Dubai cut-off the day before delivery (mirrors pauseSubscription cutoff semantics).",
    systemOnly: false,
  },
  // Day-19 / Phase 1 — bulk variants. Operators select N tasks on the
  // /tasks page (selection persistence across pagination shipped Day-17
  // PR #175/#176) and apply a uniform patch (date / time-window / address)
  // or cancel all. Each operation is transactional at the DB level + per-
  // task SF push enqueued via QStash with partial-failure UX.
  //
  // Split-perms rationale (§J-5 ruling): bulk operations are operationally
  // heavier than single-row mutations. Ops Manager owns these; CS Agent
  // does NOT auto-pickup (their explicit list omits both bulk perms).
  "task:bulk_update": {
    id: "task:bulk_update",
    resource: "task",
    action: "bulk_update",
    description:
      "Apply a uniform field patch (date / time-window / address) to many tasks in one operation. Per-task cutoff guard fires individually; tasks past cutoff fail without aborting the batch.",
    systemOnly: false,
  },
  "task:bulk_cancel": {
    id: "task:bulk_cancel",
    ...
    action: "bulk_cancel",
    description:
      "Cancel many future tasks in one operation. Same per-task semantics as task:cancel; partial-failure UX surfaces successes vs cutoff-rejected rows.",
    systemOnly: false,
  },
