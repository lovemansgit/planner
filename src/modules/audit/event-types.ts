...
const EVENT_TYPES_DRAFT = {
  ...
  "subscription.exception.created": {
    ...
    description:
      "Day 13 / T3. A subscription exception (skip / pause_window / address_override_one_off / address_override_forward / append_without_skip) was created. Emitted in the same database transaction as the originating service call. For type='skip' without skip_without_append=true, emitted alongside subscription.end_date.extended with shared correlation_id. Day 29 / §D(2) Phase-1 (plan-PR #302): for type='skip' the metadata carries outbound_emission to record whether an outbound SF cancel was enqueued. Day-56 Phase-5 (OQ-5): this now covers ALL type='skip' variants including move-to-date — the move-to-date original is CANCELLED on SF (not rescheduled), superseding the parked Phase-2 rescheduleTask path, so no skip variant leaves a stale original live.",
    metadataNotes:
      "subscription_id (uuid), exception_id (uuid), type (enum — see subscription_exceptions_type_check), target_date (YYYY-MM-DD — start_date of the exception), compensating_date (YYYY-MM-DD or null — populated only for type='skip' without skip_without_append), correlation_id (uuid). Day-29 §D(2) Phase-1 + Day-56 Phase-5 (OQ-5): outbound_emission ({ kind: 'cancel', task_id: uuid } | { kind: 'none' }) is present for ALL type='skip' variants (default skip, skip-without-append, AND move-to-date); absent for non-skip types. kind='cancel' means enqueueCancelTask was invoked post-commit with the recorded task_id; kind='none' means no outbound (task not materialized or no external_tracking_number).",
    systemOnly: false,
  },
