...
// AMENDED Day 19 (memory/decision_task_module_amendment_v1.md):
//
//   - System-actor (withServiceRole, no requirePermission):
//       bulkCreateTasks
//     Bulk creation stays system-only — no operator product surface
//     for bulk in v1; CSV bulk upload is Phase 2.
//
//   - Dual-actor (single fn, branches on ctx.actor.kind):
//       createTask
//     System-actor branch (cron + bulk-import + migration paths):
//       bypasses `task:create` via assertSystemActor — unchanged.
//     User-actor branch (operator ad-hoc creation per Love's Phase 1
//       product vision): requires `task:create`. Single wire path;
//       same validation + insert + audit emit downstream.
//     The audit event captures actor_kind so forensic queries can
//     differentiate cron-originated vs operator-originated tasks.
//
...
//
// Operator cancel path is `task:cancel` (Day-19 / Phase 1) — sets
// internal_status='CANCELED' and pushes cancel to SF outbound. Bulk
// variants (`task:bulk_cancel`, `task:bulk_update`) ship in the
// Phase 1 lane alongside this.
//
...
/**
 * Create a single task. Dual-actor gating per Day-19 amendment to the
 * Day-5 lock (memory/decision_task_module_amendment_v1.md):
 *
 *   - System actor (cron + bulk-import + migration paths): bypasses
 *     `task:create` perm via assertSystemActor. Existing behavior;
 *     zero regression for the canonical task generators.
 *   - User actor (operator ad-hoc creation per Love's Phase 1 vision):
 *     requires `task:create`. Lets operators create one-off tasks for
 *     deliveries that don't fit a subscription cadence (catering orders,
 *     sample drops, replacements).
 *
 * Single fn with single wire path — same validation + insert + audit
 * emit downstream of the actor branch. The audit event captures
 * actor_kind so forensic queries can differentiate cron-originated
 * vs operator-originated tasks (`SELECT * FROM audit_events WHERE
 * event_type='task.created' AND actor_kind='user'`).
 *
 * `bulkCreateTasks` stays system-only — no operator product surface
 * for bulk creation in v1; CSV bulk upload is Phase 2.
 *
 * Throws:
 *   - ForbiddenError    user actor lacks `task:create`.
...
export async function updateTask(
  ...
  const normalised: UpdateTaskPatch = {
    ...
    addressId:
      patch.addressId !== undefined
        ? requireNonEmpty(patch.addressId, "addressId")
        : undefined,
  };
  ...
  const result = await withTenant(tenantId, async (tx) => {
    ...
    compareScalar("addressId", before.addressId ?? undefined);
    ...
    for (const field of changedFields as (keyof UpdateTaskPatch)[]) {
      ...
      switch (field) {
        ...
        case "deliverToCustomerOnly":
          ...
          break;
        case "addressId":
          toApply.addressId = normalised.addressId;
          break;
        default: {
