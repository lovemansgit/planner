...
**Operating model:** `memory/BUILD-METHODOLOGY.md` (three-role; §2 floors drift-exempt) — read at bootstrap.
...
- `subscription.pause_cancels_pushed` (v1.17) — body includes `subscription_id`, `correlation_id`, `pushed_task_count`, `enqueued_count`, `failed_chunks`, `pause_start`, `pause_end`. Outbound leg of bounded pause (R2, plan-PR #337 §2.R2); paired with `subscription.paused` via shared `correlation_id`. Emitted only when ≥1 pushed task is in the pause window.
- `subscription.resume_reactivations_pushed` (v1.21) — body includes `subscription_id`, `correlation_id`, `actual_resume_date`, `reactivated_task_count`, `enqueued_count`, `failed_chunks`, `previous_awbs`. Outbound leg of resume re-sync (R16, plan #408); paired with `subscription.resumed` via the pause window's `correlation_id`. Emitted only when ≥1 restored task carried an SF AWB (re-created fresh on SF — cancel is terminal vendor-side); partial fan-out failure emits-then-re-throws (R2 posture).
- `task.note_pushed_to_external` (R3, shipped Day-52; registered here in the v1.18 catch-up) — body includes `task_id`, `awb`, `correlation_id`. Outbound leg of the driver-note workflow; paired with `task.note_added`. Emitted only when the task has a live SF AWB. Note text NEVER in the body (PII).
- `task.address_override_pushed` (v1.18) — body includes `task_id`, `awb`, `exception_id`, `address_override_id`, `correlation_id`. Outbound leg of the one-off address override (R4); paired with `subscription.exception.created` + `subscription.address_override.applied` via shared `correlation_id`. Emitted only on a successful enqueue for a pushed task. Address text NEVER in the body — IDs only.
- `subscription.address_override_pushed` (v1.18) — body includes `subscription_id`, `exception_id`, `address_override_id`, `correlation_id`, `backfilled_task_count`, `pushed_task_count`, `enqueued_count`, `failed_chunks`, `effective_from`. Outbound leg of the forward address override (R5); paired with `subscription.address_override.applied` via shared `correlation_id`. Emitted only when ≥1 pushed task is in the in-horizon window; partial fan-out failure (`failed_chunks > 0`) emits-then-re-throws (R2 posture).
...
- `credentials.method_changed` — body includes `tenant_id`, `previous_method`, `new_method`, `credentials_cleared` (always true). Same sensitive-by-class minimal shape as `credentials.set`; **NEVER plaintext, NEVER Vault IDs** (v1.19)
- `user.password_reset` (v1.29) — body includes `email`. An admin reset a user's password from the `/admin/users` surface (F5; `resetUserPassword`, gate `user:update`, cross-tenant `merchant:read_all`). The plaintext password **NEVER appears in the body under any encoding** (not hashed, not prefixed, not first-N-chars). Sets a new password on `auth.users` via the admin SDK; no `public.users` mirror change.
...
10. For address overrides: update affected tasks' `address_id` per scope (v1.18, R4/R5 rulings; Day-53 horizon confirmation): one-off = the single task at the date; forward = EVERY upcoming task on THIS subscription (`delivery_date >= start_date`, no upper date bound — full materializer horizon per Love's Day-53 confirmation; sibling subscriptions untouched — subscription-scoped, NOT consignee-scoped). Pushed rows flip `outbound_sync_state='pending_update'` (migration 0029)
10b. (v1.18) Post-commit SF outbound push for the override: one-off enqueues a single `task-resource:update` via `enqueueUpdateTask`; forward fans out via `enqueueBulkUpdateTasks` (one message per pushed upcoming task). The wire patch carries a SERVER-BUILT ConsigneeSnapshot read from the override `addresses` row + `consignees` row (option B per the Day-52 ruling on `memory/followup_address_edit_sf_outbound_gap.md`). Not-yet-materialized future dates need no push — the exception row is read by the materializer CTE forward branch and they materialize at the new address
...
**`changeConsigneeCrmState` — churn hard-stop cascade (v1.26, Day-54 R-E ruling):** the CHURNED transition is a HARD STOP on everything. In the same transaction: every subscription of the consignee ends (active AND paused); never-pushed non-terminal tasks cancel locally; pushed non-terminal tasks — **including driver-bound ASSIGNED/IN_TRANSIT rows, the single sanctioned bypass of the §3.1.8 assignment freeze (v1.25)** — enter vendor recall (`outbound_sync_state='pending_cancel'`) WITHOUT a local status flip. Honesty rule: a pushed task reads CANCELED only on vendor confirmation (webhook); a vendor-refused recall keeps its true status, lands the cancel-DLQ `'failed'` signal, and surfaces as "Vendor refused recall — final delivery". The transition sits behind a mandatory warning popup covering the hard stop and the recall attempt. Post-commit: `consignee.churn_cascade` audit event (§3.1.2) + the recall fan-out via the existing cancel pipeline. By contrast (ruled in the same firing): **subscription cancellation stays softer — pending tasks continue to completion** (verified existing behavior); **task cancel = that task only**. Non-churn CRM transitions cascade nothing. **Role gate (v1.27, Day-54 ruling — "churn should only happen from merchant level, not transcorp admin"):** the CHURNED transition requires `consignee:churn` in addition to `consignee:change_crm_state`. The new permission is granted to the merchant-level roles only (Tenant Admin, Ops Manager, CS Agent) and is the single carve-out from transcorp-sysadmin's otherwise-ALL permission set; the service gate rejects before any DB work, and the CrmStateModal hides the Churned option from actors without it.
...
- `target_date_override` (move-to-date): operator moves the delivery to a chosen date. Validates the date is an eligible `days_of_week` weekday and not in blackout/pause; otherwise reject. **v1.30 (D56, Love "build them properly, do not paper over it"):** move-to-date **cancels the original on SuiteFleet AND materializes a real one-off task at the chosen date** (so SF holds exactly ONE delivery) — it is NOT a reschedule (the parked Aqib-gated `rescheduleTask` path is superseded) and NOT a tail-end fan-out (`end_date` is NOT extended; the single moved task is materialized via the no-cap one-off primitive, so the cron does not fan out intermediate days). **Move-to-date means "move to a date that ISN'T already delivering": a within-schedule target is REJECTED at the service layer** — for a bounded subscription that is `target <= end_date`; for an open-ended subscription (`end_date IS NULL`) EVERY eligible day already delivers, so any move-to-date is rejected (use `skip_without_append` to cancel instead). A consignee address gap at the target rolls the whole operation back (the original is never cancelled without a replacement). **Timeline move-link (§3.3.6):** the move is surfaced in the task timeline drawer both directions via two typed per-task audit events (no schema change) — the new delivery's drawer shows "Moved from [old date] / replaces AWB [old AWB]"; the cancelled delivery's drawer shows "Moved to [new date] / see AWB [new AWB]". The new task's AWB is assigned by SuiteFleet only after the async push, so it is resolved at read time from the new task row (AWB-pending until then). Drawer gate unchanged (`task:view_timeline`).
...
Redefined per Love's Day-54 R-A ruling (v1.25): **the 18:00 cut-off applies ONLY to order creation — it is the deadline for creating orders.** Creation-class flows keep the gate: order creation (materializer + manual paths), `skip` exceptions (skips create/cancel orders by date — before cut-off apply immediately, after cut-off rejected with clear error), and `append-without-skip`.

**Editability is gated by assignment alone:** if a delivery is not assigned, edits and cancellations are allowed at any hour; once a driver holds it (`ASSIGNED`, and onward through `IN_TRANSIT`), no edits or cancellations — terminal states likewise reject. Driver NOTES are communication, not edits: deliverable until a terminal state. Address overrides are edits and follow the assignment gate. A pause excludes driver-bound tasks from its window-cancel (the excluded count is audited); the R-E churn cascade is the single sanctioned bypass of the assignment freeze. This restores the Day-3 lock-at-assignment decision (`memory/decision_task_editability_cutoff_at_assigned.md`) and supersedes the v1.16 time-based-editability rule.
...
**Status renders distinctly — coarse + fine model (v1.31, A2 mandate; Love-ruled scope expansion):** Every SF courier state renders **distinctly** on Planner — **no collapsing**. The 15 actions above are **14 lifecycle states** (the 15th, `TASK_HAS_BEEN_UPDATED`, is a non-lifecycle edit → renders nothing) once `ARRIVED_ON_DC`/`ARRIVED_IN_DC` fold as one `ARRIVED_AT_DC` (one state spelled two ways across the action vs status-field vocabularies). The 14: `ORDERED · ASSIGNED · PICKED_UP · ARRIVED_AT_DC · IN_TRANSIT · HUB_TRANSFER · OUT_FOR_DELIVERY · DELIVERED · FAILED · PROCESS_FOR_RETURN · RETURNED_TO_SHIPPER · CANCELED · RESCHEDULED · REATTEMPT`. The distinction is carried by a new nullable `tasks.courier_status` column (fine, render-only) kept **separate from** the unchanged 8-value `internal_status` coarse lifecycle (which all business logic — editability, pause/resume fan-out, churn cascade, move-to-date, transition guards — keeps reading). Render surfaces read `courier_status`, falling back to the coarse map when NULL (Planner-only states — SKIPPED, manual cancel — and pre-backfill rows). The SF→internal mappers (`status-mapper.ts`, `status-progression.ts`) gain sibling fine maps + a fine-advance guard (in-transit ramp rank `PICKED_UP < ARRIVED_AT_DC < IN_TRANSIT < HUB_TRANSFER < OUT_FOR_DELIVERY`) so the 5 in-transit sub-states progress on the fine field while coarse stays `IN_TRANSIT`. The calendar projection (`DayDisplayStatus`) stops folding/mislabeling — in-transit and out-for-delivery are two distinct statuses. Operators filter by **all 14 fine states via a dropdown** (not the coarse 7) on **both `/tasks` and the calendar** (Love ruling; URL-state recommend a new `?courier_status=` param). `ARRIVED_AT_DC` renders the operator label **"Arrived in DC"** (DC = Distribution Centre; Love's term — internal value unchanged). Per-state label + icon + family-colour per §3.3.11. Plan: `memory/plans/day-56-phase-8-status-distinct-render.md`; decision: `memory/decision_d56_phase8_status_distinct_render.md`.
...
- `/admin/merchants/new` — create form: name, slug, pickup address (street, district, city)
- `/admin/merchants/[id]/edit` — edit form (v1.12 amendment) mirroring the `/new` component pre-filled from the current `tenants` row. Editable fields: name, slug, pickup address (line/district/city), SF customer code, **SF region** (v1.14 — select from active rows in `suitefleet_regions`). `tenants.status` is intentionally read-only here — activate/deactivate remain their own row actions. Credentials are NOT editable here — see `/admin/merchants/[id]/credentials` below. Slug-change confirm dialog gates submit. Permission: `merchant:update` (transcorp-sysadmin only).
...
- Default view: Month (Day-53 Love ruling — all calendar views default to monthly; supersedes BRD §6.2.1's week default. Already in effect here: R9 removed the consignee Week view and falls back to Month. See §9 v1.20.)
...
- **Outbound-sync badge** (v1.18 extension) — "Sending to SuiteFleet" renders while an operator address override's SF update is in flight (`outbound_sync_state='pending_update'`, migration 0029); joins the existing `pending_cancel` / `failed` badge family
- Action buttons (contextual, permission-gated):
  ...
  - Change address for this delivery only — for `subscription:change_address_one_off` (v1.18: backfills the task + pushes SF; NO confirmation popup — scope self-evident per the R4 ruling)
  - Change address from this delivery onwards — for `subscription:change_address_forward` (v1.18: backfills all in-horizon tasks on the subscription + fans out SF pushes; gated by an INLINE modal-within-popover confirmation whose copy is ruling-VERBATIM: "Are you sure you want to update the address for all future tasks on this subscription?")
...
**Calendar grid:**
- Month view default; Week and Day views available (Day-53 Love ruling — monthly default everywhere; supersedes BRD §6.4 / §6.2.1 week default. See §9 v1.20.)
...
| Snow White | `#FAF8F4` | Warm brand white / framing reference (was the page surface; the live page field is now white-dominant — see *Page field* note below, Phase 10 / v1.34) |
...
| Name | Hex | Use |
|---|---|---|
| Paper | `#FCFBF8` | **Default page field — white-dominant (Phase 10 / v1.34; was `#FAF8F4`)**. See *Page field* note below. |
...
**Page field — white-dominant (Phase 10 · v1.34):** The default page field is **`#FCFBF8`** (`--color-surface-primary` / `--color-paper`), a warm near-white — lifted from the original Snow White cream `#FAF8F4`, which read dull next to white (Love's standard: WHITE dominates the field). Cream's warmth recedes to thin framing/borders/section breaks (Ivory `#F2EEE6`, Stone 200, navy-alpha borders). The value stays *faintly* warm (R>G>B) and uniformly below the Phase-9 B+ card surface `#fffdf8` on R/G, so floating cards (`--color-b-card` + the Phase-9-sanctioned `--shadow-b-card` elevation) still read as floating — the field is never stark `#FFFFFF` (the warm card would go dingy). Snow White `#FAF8F4` is retained as the warm brand-white / framing reference. This is a central token + route-group-shell change (no per-screen edits); navy/green/amber/red, status-badge + LED tones, and the composition ratio (white still ~58%) are unchanged. Filed: `memory/decision_phase10_white_dominant_surface.md`.
...
**Delivery-status render — colour families + label/icon (v1.31; §3.1.10 fine `courier_status`):** Each of the 14 distinct SF courier states renders distinctly via the **combination** of a family colour + an icon + a label — never colour alone (no new hex, no palette widening). Families: **Neutral** Stone 200/600 (`ORDERED`, and `CANCELED` line-through) · **Info** Ocean Blue `#1F6FA8` (`ASSIGNED`) · **Amber ramp** = the in-transit journey, `OUT_FOR_DELIVERY` on the hi-vis **core Signal Amber** `#E8A33C` (highest-attention, Love-locked) with `PICKED_UP`/`ARRIVED_AT_DC`/`IN_TRANSIT`/`HUB_TRANSFER` demoted down the Amber-100/300/600/Deep rungs · **Success** Grass Green `#3e7c4b` (`DELIVERED`) · **Alarm** Bright Red `#D93A2B` (the failure family `FAILED`/`PROCESS_FOR_RETURN`/`RETURNED_TO_SHIPPER`, disambiguated by icon+label) · **Hold** Stone 600 on Ivory (the `RESCHEDULED`/`REATTEMPT` pair, disambiguated by icon+label, per the ON_HOLD token above). Six new icons (`PickupIcon`, `DcIcon`, `HubTransferIcon`, `OutForDeliveryIcon`, `ReturnIcon` outline+solid, `RescheduleIcon`, `RetryIcon`) join the existing `PackageIcon`/`VanIcon`/`TruckIcon`/`PodIcon`/`CautionIcon`. `ARRIVED_AT_DC` renders the label **"Arrived in DC"** (Love's term). Operators filter by all 14 fine states via a **dropdown** (`?courier_status=`) on `/tasks` and the calendar — distinct from the family-grouped legend, which is a separate control. Full per-state table in `memory/plans/day-56-phase-8-status-distinct-render.md` §3.
...
- `auth_method` is IMMUTABLE after region creation (v1.15) and remains so. v1.19 adds the PER-MERCHANT escape hatch instead of region mutation: `tenants.suitefleet_auth_method_override` (nullable; NULL = region default; migration 0030). The resolver uses the EFFECTIVE method (`override ?? region.auth_method`) to interpret the credential pair. Switching a merchant's method via `setMerchantAuthMethodOverride` clears its Vault pair in the same transaction — a half-switched merchant fails LOUD on outbound (`credentials_not_configured` → failed-push) until new credentials are entered for the new method; audited via `credentials.method_changed`. Context: Love's 2026-06-11 correction — SuiteFleet accepts BOTH methods on ALL tenants, sandbox included; "sandbox is OAuth-only" was a Planner-side assumption. The v1.15 region seed assignments stand as DEFAULTS, not vendor constraints.
...
| Public marketing landing route ownership: does the public landing take `/` (displacing the §3.3.9 authenticated operator landing to a post-login home / redirect) or live on a separate route with `/` staying the authed home? (v1.32 — was blocking the landing-page build) | **RESOLVED (Day 58, v1.33 — Love's ruling):** public marketing landing lives at **`/welcome`**; `/` STAYS the authenticated operator home (§3.3.9 unchanged). Landing build no longer route-blocked. |
...
| v1.17 | 9 Jun 2026 (Day 51, calendar-management Phase 1 PR-2 §3.6 clearance) | **R2 — bounded pause now fans out SF cancels (outbound leg).** Before: `pauseSubscription` canceled in-window tasks locally only (`internal_status='CANCELED'`), leaving pushed tasks live on SuiteFleet so drivers still dispatched on paused days (calendar-management lane diagnostic R2). PR-2 (plan-PR #337 §2.R2): `markTasksCanceledInWindow` now also flips PUSHED rows (non-null `external_tracking_number`) to `outbound_sync_state='pending_cancel'` and `RETURNING (id, external_tracking_number)`; post-commit `pauseSubscription` fans the pushed subset out via `enqueueBulkCancelTasks` (the AWB single-cancel fan-out, NOT a numeric bulk-PATCH route) and emits a new `subscription.pause_cancels_pushed` audit event (§3.1.2 — paired with `subscription.paused` by shared `correlation_id`). Per Q5, partial fan-out failure (`failedChunks > 0`) emits-then-re-throws so the caller surfaces "saved locally; SF cancels pending"; DLQ-on-chunk-failure deferred. Early manual resume reconciles the safe-state half (`pending_cancel`→`synced`); active SF re-activation on resume is filed as the R16 follow-on (lane diagnostic). No migration — `pending_cancel` exists since migration 0026. **Header version-pointer correction:** the `**Version:**` line read v1.15 (it was not advanced when v1.16 shipped Day-30 via PR #308); corrected to v1.17 in this bump. The §9 amendment-log table is authoritative and already carried the v1.16 row — no prior amendment-log rows were altered. |
| v1.18 | 10-11 Jun 2026 (Day 52 overnight build + Day-53 horizon correction, calendar-management Phase 1 PR-4/PR-5; Love's Day-52 rulings per `memory/decision_d52_calendar_phase1_r4_r5_rulings.md` incl. its Day-53 correction note) | **R4 + R5 — address overrides now backfill tasks and push SuiteFleet (outbound legs).** Before: both popover address-override actions wrote the exception row only — upcoming tasks kept the old address locally and SF was never told (diagnostic A2.4/A2.5). R4 (one-off): `addSubscriptionException` backfills the single task's `address_id` in-tx (`markTaskAddressOverridden`) and post-commit enqueues one SF update; NO confirmation popup (ruled). R5 (forward): backfills EVERY upcoming task on the subscription (`delivery_date >= start_date`, no upper date bound — Love's Day-53 confirmation: the Day-52 ruling's 14-day figure was stale framing from before the Day-28 horizon bump to `MATERIALIZATION_HORIZON_DAYS = 21`; a literal 14-day bound would strand materialized tasks 15-21 days out on the old address, since materializer INSERTs are ON CONFLICT DO NOTHING; subscription-scoped NOT consignee-scoped) via `markTasksAddressOverriddenForward` and fans out via `enqueueBulkUpdateTasks`; INLINE confirmation popup with ruling-verbatim copy (OQ-3); not-yet-materialized future dates pick the override up from the exception row via the materializer CTE forward branch (no subscription column). Both wire patches carry a SERVER-BUILT ConsigneeSnapshot (`buildConsigneeSnapshotForAddress`; option B ruling — also folded into `updateTaskAndPushOutbound`, making the /tasks address edit push SF and RETIRING the B2 OQ-3 "next scheduled push pass" disclosure copy). OQ-1 ruled (a): migration 0029 extends `outbound_sync_state` with `'pending_update'`; the calendar badge renders "Sending to SuiteFleet"; `/api/queue/update-task` converges it to `'synced'` on SF 2xx and `update-task-failed` flips it to `'failed'`. New §3.1.2 events: `task.address_override_pushed` (R4) + `subscription.address_override_pushed` (R5); `task.note_pushed_to_external` (R3, shipped Day-52) registered in the same §3.1.2 catch-up. NOTE: the OQ-5 ruling's "§3.1.4/§3.5 outbound surface listing" pointer maps to §3.1.4 + §3.3.3 in this brief's numbering (the plan's "§3.5 task action model" label does not match this brief's §3.5, which is label generation); both sections amended accordingly. |
| v1.19 | 11 Jun 2026 (Day 53/54 night, sandbox api_key enablement lane; Love's correction + directives per `memory/decision_d53_night_sandbox_apikey_lane.md`) | **Per-merchant SF auth-method override.** Love's correction: SuiteFleet accepts BOTH auth methods on ALL tenants, sandbox included — "sandbox is OAuth-only" (v1.15 narrative, `decision_aqib_api_key_auth_header_verified.md` probe-gate §) was a Planner-side assumption, not a vendor limitation; both records annotated append-only. New `tenants.suitefleet_auth_method_override` (migration 0030, nullable, NULL = region default); resolver discriminates on the EFFECTIVE method (`override ?? region.auth_method`); `suitefleet_regions.auth_method` stays IMMUTABLE (the v1.15 ruling is narrowed, not unwound — the override is the per-merchant exception the v1.15 "future enhancement" anticipated, minus region mutation). Switching clears the merchant's Vault pair in-tx (fail-LOUD between states; no silent fallback) and emits the new `credentials.method_changed` event (§3.1.2; OQ-8 sensitive-by-class shape). Admin: `/admin/merchants/[id]/credentials` gains the method-switch control with consequence-explicit confirm; form labels follow the effective method. Sandbox region + all existing merchants stay OAuth by default (ruling 5: only Demo Bistro switches, on Love's clearance, as the api_key wire-proof merchant — runbook at `memory/runbooks/day-54-demo-bistro-apikey-proof.md`). Filed at `memory/decision_d53_night_sandbox_apikey_lane.md` + plan `memory/plans/day-54-sandbox-apikey-method-switch.md` (#387). |
| v1.20 | 11 Jun 2026 (Day 53/54 NIGHT, UI/UX pre-UAT polish lane) | **All calendar views default to MONTHLY (Love ruling).** Supersedes the BRD §6.2.1 / §6.4 week default. §3.3.3 (consignee detail calendar): default corrected Week → Month — already in effect (R9 removed the consignee Week view, falls back to Month); brief catches up. §3.3.4 (consolidated `/calendar`): default changed Week → Month in code (`parseView` no-param landing; `src/app/(app)/calendar/page.tsx`). NO view removed and NO toggle removed — Week and Day remain available via explicit `?view=` deep-links (which keep working unchanged); only the no-param landing default changes. The consolidated MonthAnchorNav already carries Previous / This month / Next, so the new default landing is fully navigable. Owner-ruled exception to the Tier-1 "no flow changes" line for this UI-polish lane; the run-sheet re-walk re-verified every calendar step against the new month landing. Ruling source: Love's Day-53 calendar-default addendum (verbatim in `memory/uiux_audit_day53.md` lane memo). (Also corrects the closing-line pointer, which read "End of v1.18" after the v1.19 bump — append-only table rows unchanged.) |
| v1.21 | 11 Jun 2026 (Day 53 PM, Session C add-a-second-address lane; Love's ruling cleared-by-firing in the Day-53 PM dispatch) | **v1.11 multi-address UI deferral PARTIALLY RETIRED — add-address only.** Operators can now add a NON-PRIMARY address from the consignee detail page (the surface the §3.3.1 onboarding copy always promised: "Add more from the consignee detail page after onboarding"). New `createAddress(ctx, consigneeId, input)` service surface in `src/modules/addresses` (permission: `consignee:update` piggyback per the v1 address-ops convention; tenant-scoped existence probe → NotFoundError on cross-tenant/missing consignee; `is_primary` hard-set false so the migration-0014 partial UNIQUE is unreachable from this surface and routing defaults never change implicitly). New Overview-tab Addresses section (all rows, primary badged) + Add-address dialog on `/consignees/[id]` (§3.3.3), gated identically to the Edit affordance. New §3.1.2 audit event: `consignee.address.added` (consignee_id, address_id, label, is_primary=false; post-commit emit per the consignee.created idiom). The new address is immediately consumable by the R4/R5 address-override pickers (closes the reachability gap in `memory/followup_no_ui_second_consignee_address.md` for the add path) and by per-weekday rotation. **The REST of the v1.11 deferral STANDS** (reviewer-ruled on plan-PR #402, Love-confirmed): edit-address / delete-address / set-primary / rotation-pattern changes remain Phase-2-deferred — §4's multi-address rotation row narrows accordingly rather than retiring. NO schema delta (plain INSERT into the existing `addresses` table). Plan: `memory/plans/day-53-session-c-add-second-address.md` (#402); code: #405. |
| v1.22 | 11 Jun 2026 (Day 53 PM, Session C R12 lane; Path B dispatch-ruled, brief bump dispatch-assigned) | **New surface: resolved failed-pushes review page.** `/admin/failed-pushes/resolved` — read-only history of resolved DLQ rows (resolved-at DESC, resolver email via LEFT JOIN users with NULL rendering as "System" for system-resolved rows, resolution notes, failure context). Closes the Day-33 operator-visibility gap (`memory/followup_resolved_rows_visibility_gap.md`): after PR-D's bulk-resolve shipped, resolved rows vanished from the UI and resolution could only be verified by developer SQL. New `listResolvedFailedPushes` service fn gated `failed_pushes:retry` (the work-queue page's own gate — deliberately NOT the Day-30 `failed_pushes:read` split, which keeps notes/failure context Tenant-Admin-only); new `listResolvedByTenant` repository query; server-component-only page (zero actions — the work-queue stays the only mutation surface); discoverability via a "View resolved" link on `/admin/failed-pushes` (deliberately NOT a nav-config entry — Session B's R6 nav lane in flight). NO schema delta, NO new permissions, NO audit events (read-only per R-4). Plan: `memory/plans/day-53-session-c-r12-resolved-rows.md` (#409). NOTE: renumbered v1.21 → v1.22 per Love's Day-53 EVE batch ruling ("Merge order: #405 first (keeps v1.21); #412 renumbers to the then-next-free number before merge per the recorded fixup") — #405's add-address amendment takes v1.21; this row is the next free after it. |
| v1.23 | 11 Jun 2026 (Day 53 PM, Plan-A wave Lane A; Love's Plan-A ruling per `memory/decision_d53_plan_a_pre_uat_queue.md`) | **R16 — resume now re-activates SF-cancelled deliveries (outbound leg; closes the v1.17 R16 follow-on).** Before: early manual resume restored tasks locally and reconciled the safe-state half only (`pending_cancel`→`synced`) — deliveries already cancelled on SuiteFleet stayed cancelled vendor-side, so no driver dispatched until a fresh create happened to re-materialise one. Probes first (spec mandate, Demo Bistro probe tasks): SF cancel is TERMINAL via the API client (`{status:"ORDERED"}` un-cancel PATCH → 403), and a fresh create reusing the SAME `customerOrderNumber` is ACCEPTED with a new AWB — so re-activation = re-create. `markTasksRestoredInWindow` now clears `external_id`/`external_tracking_number`/`pushed_to_external_at` and flips SF-cancelled restored rows to `outbound_sync_state='pending'` (CTE returns the pre-clear AWB); post-commit `resumeSubscription` re-enters those rows into the EXISTING push pipeline via `enqueueTaskPushBatch` (fresh SF create; `markTaskPushed` installs the new AWB) and emits the new `subscription.resume_reactivations_pushed` event (§3.1.2 — paired with `subscription.resumed` by the pause window's `correlation_id`; carries `previous_awbs` forensics). R2's emit-then-re-throw posture verbatim on partial enqueue failure. Late webhooks for the old AWB find no row (lookup by the cleared `external_tracking_number`) — the restored task can no longer be clobbered back to CANCELED. No migration — `'pending'` exists since 0026/0028. Plan: `memory/plans/day-53-r16-resume-sf-reactivation.md` (#408). UAT run-sheet step-H known-limitation note retired in the same PR. NOTE: renumbered pre-merge per the recorded fixup rule, twice — v1.22 → v1.21 on Day-53 EVE (when #405/#412 were still open) and v1.21 → v1.23 on Day-54 (Session C's #405 took v1.21 and #412 took v1.22 on merge; this row takes the next free number against current main). |
| v1.24 | 11 Jun 2026 (Day 53/54 overnight, click-reduction lane; Love-approved #431, brief bump dispatch-assigned) | **`/tasks` filter controls consolidated to one compact row.** The three previously-stacked full-width filter blocks on the operator `/tasks` page — status pills, date-range, and search — collapse into a single wrapping flex row, so the task table sits higher on the page instead of below three stacked blocks. **No filter removed, no behaviour change, identical URL state** (`?status` / `?q` / `?from` / `?to` / `?perPage` all unchanged). Page-level layout only: the shared `SearchBar` / `DateRangeFilter` components are untouched (they serve 8 pages); the page neutralises their stacking margins via `[&>*]:mb-0`. The hero-count band is repositioned above the filter row but its **treatment is unchanged** — hero-numeral unification is deferred to the future `<HeroCount>` extraction per the Day-53 EVE ruling (`memory/decision_d53_bignumber_defer_to_herocount.md`). Plan: `memory/plan_click_reduction_tasks_filter_collapse.md` (#431). NOTE: v1.24 assigned next-free against current main at build time; re-confirm / renumber at merge-prep per the recorded fixup rule if a peer bump lands first. |
| v1.25 | 12 Jun 2026 (Day 54, Session C R-A lane; Love's ruling rode the Day-54 firing — "the 18:00 cutoff applies ONLY to order creation … Editability is gated by assignment alone … Restores the Day-3 lock-at-assignment ruling; reverts v1.16; §3.1.8 redefined") | **Cut-off redefined as the order-creation deadline; editability locks at assignment (v1.16 SUPERSEDED, Day-3 ruling RESTORED).** §3.1.8 rewritten: creation-class flows (materializer, manual creation, `skip` exceptions, `append-without-skip`) keep the 18:00 gate; the seven task-edit/cancel service sites + the pause start-date drop the time gate and take the assignment gate (`isTaskEditable`: editable = CREATED/ON_HOLD/SKIPPED; locked = ASSIGNED/IN_TRANSIT; terminal rejects). Reviewer-ruled refinements (plan #464 r1): driver notes are COMMUNICATION (no time/assignment gate; terminal-only guard) and address overrides are EDITS (assignment-gated, hour-unlimited on unassigned tasks). Bulk cancel legs (`markTaskSkipped`, `markTasksCanceledInWindow`, forward-override backfill) exclude driver-bound rows; skip/one-off on a frozen delivery REJECT (honest state over silent no-op); `subscription.paused` metadata gains `assigned_tasks_excluded`. Calendar day-popover: ASSIGNED leaves `MUTATION_ELIGIBLE_STATUSES`; driver-bound days render a plain locked explanation; add-note survives. The v1.16 row above is UNTOUCHED (append-only); its `followup_assigned_before_cutoff_dispatch_race.md` is closed by this amendment. The R-E churn cascade (separate lane) is the single sanctioned bypass of the freeze. Plan: `memory/plans/day-54-session-c-ra-assignment-gate.md` (#464). NOTE: v1.25 assigned next-free against current main at build time; re-confirm/renumber at merge-prep if a peer bump lands first. |
| v1.26 | 12 Jun 2026 (Day 54, Session C R-E lane; Love's churn ruling rode the Day-54 firing — "churn is a HARD STOP on everything, assigned deliveries included … behind a mandatory warning popup … Subscription cancellation stays softer … Task cancel = that task only") | **Churn hard-stop cascade.** `changeConsigneeCrmState(toState='CHURNED')` now cascades in-tx: all subscriptions end (active + paused), never-pushed non-terminal tasks cancel locally, pushed non-terminal tasks (incl. driver-bound — the single sanctioned bypass of v1.25's assignment freeze) flip to `outbound_sync_state='pending_cancel'` with internal_status PRESERVED (honesty rule: vendor-confirmed cancels only, via the existing webhook applier; refused recalls keep accurate state, land the existing cancel-DLQ 'failed' signal, and render "Vendor refused recall — final delivery" on the calendar). New §3.1.2 event `consignee.churn_cascade` (counts + recall AWBs + correlation_id shared with the fan-out payloads). Mandatory warning popup in CrmStateModal (hard-stop + recall wording; confirm reads "Churn — stop everything"). Dispatch-ordered verification recorded: `endSubscription` already matches the ruled softer behavior (pure status flip, no task cancellation, no push-pipeline subscription filter) — NO drift. MP-13 named test Path 2 rewritten from gap-pin to this contract. §3.1.4 body amended. No schema delta. Plan: `memory/plans/day-54-session-c-re-churn-cascade.md` (#476). NOTE: v1.26 assigned assuming the same-session sibling v1.25 (#472, R-A) merges FIRST per the recorded clearance order — this branch carries no v1.25 row, so whichever PR merges second resolves the §9 conflict keeping BOTH rows and re-verifies the additive-only diff at merge-prep per the recorded fixup rule. |
| v1.27 | 12 Jun 2026 (Day 54 PM, Session A; Love's ruling, verbatim: "churn should only happen from merchant level, not transcorp admin") | **Churn role gate.** New permission `consignee:churn` required IN ADDITION to `consignee:change_crm_state` for the CHURNED transition (the v1.26 hard-stop cascade). Granted to Tenant Admin (TENANT_SCOPED auto-pickup), Ops Manager (`permsFor("consignee")` auto-pickup), CS Agent (explicit add); **excluded from transcorp-sysadmin** — the single carve-out from that role's every-permission posture (the role-catalogue invariant test updated to pin the exclusion). Server gate in `changeConsigneeCrmState` rejects ForbiddenError before any DB work; `CrmStateModal` hides the Churned radio for actors without the permission (visibility only — server re-checks). Verification that drove the build: transcorp-sysadmin's ALL set reached the CHURNED branch server-side pre-v1.27; tenant-scoping only incidentally limited it. No schema delta; permission catalogue + roles + one service gate + UI prop. §3.1.4 churn paragraph amended additively. |
| v1.28 | 13 Jun 2026 (Day 54/55, Session B bag-tracking reports stack; Love's plan clearance #502 "ALL TEN recommendations accepted", re-walk sign-off "Re-walk PASS … cleared to merge", and collapse ruling "Collapse via #513 … the 4-PR split was a build-time review device") | **New feature: bag-tracking (asset-tracking) reports.** Builds on the Day-6/B-2 read-through `asset_tracking_cache`. Three schema deltas: **0032** `asset_scan_log` (append-only per-status scan history; enforced by a BEFORE UPDATE OR DELETE trigger raising P0001, NOT a 0002-style rule — the known cascade-conflict lesson; RESTRICT FKs + an `app.allow_scan_log_delete` session-GUC escape hatch for teardown only; `vendor_scanned_at` nullable + `received_at` NOT NULL — Love's timestamp ruling: "if no timestamp then put actual timestamp of receiving the data", surfaced as "recorded in Planner"); **0033** adds `SORTED` to the `asset_tracking_cache` state CHECK (Aqib-confirmed the enum is complete at five: Collected / Received / Sorted / EnRoute / Returned); **0034** `tenants.task_asset_tracking_enabled` (boolean, **default false — the dark switch**, gates ALL surfaces incl. nav; flips per-tenant by Love's sentence only) + `default_task_asset_type`. New surfaces: admin **Asset Tracking** report (merchant × delivery-date + rollup), admin **Inventory** (all-merchants section-per-lit-merchant view — walk finding F1 — and per-merchant), merchant **Inventory** (`/reports/inventory`, by-date + expandable by-consignee), admin **Asset Log** (append-only scan history; the report's "Allocated Asset" links here); every report value drills to the tasks list filtered to its exact AWB set (`?awbs=`). Reports grouped under a **Reports** nav dropdown (walk finding F2 — Q1's approved group; admin nav overflow repair, UI-only). Semantics in tooltips verbatim (Aqib): Allocated Asset = bags allocated to the AWB; Supp. Quantity = ice packs. Ingest: a **30-minute QStash schedule** (`*/30 * * * *`) POSTs `/api/cron/asset-tracking-poll` (signature-gated by the EXISTING QStash keys — no new secrets; $0 at 48 msg/day on the free tier per Love's cost constraint), plus a bounded manual **"Refresh now"** override (reuses the poll's caps/chunks; dark tenants refused 403; #509 cooldown — Love-ruled "accept for pilot, no cooldown now"). Permissions: `asset_tracking:read` (pre-existing) gates the merchant report; **`asset_tracking:read_all`** added (systemOnly, in `API_KEY_FORBIDDEN_PERMISSIONS`) gates the admin reports + Asset Log; CS Agent explicitly granted `asset_tracking:read`. Reads are NOT audited (R-4 posture); the poll reuses the existing `asset_tracking.*` event family with `trigger_source: poll`. Shipped **DARK** (fleet all-dark; no tenant lit) — a real tenant's flag flips only on Love's explicit per-tenant sentence AFTER Aqib's sandbox scans prove the 30-minute ingest end-to-end. Migrations 0032/0033/0034 were applied to the production Supabase as the accepted Day-54 state ([[authorization-scope-literal]] breach record). Plan: `memory/plans/day-54-session-b-bag-tracking-reports.md` (#502). Shipped via #513 squash `d02b198` (collapse-via-#513; P1 #507 / P2 #508 / P3 #509 / P4 #513 each independently reviewer-APPROVED, closed shipped-via); promoted dark to production `e00a4ae` (#515, `dpl_9EGTCnv8…`, rollback anchor `d64e835`). QStash schedule registration outstanding (needs `QSTASH_TOKEN`; deferrable to the first real-tenant flip — a dark fleet has nothing to poll). UAT: `memory/uat_addendum_bag_tracking.md` (legs 1-2 proven on the preview walk; leg 3 live-SF UAT-opportunistic pending Aqib). |
| v1.29 | 19 Jun 2026 (Day 55, overnight Builder-2 safe-build wave; Love's firing clearance, verbatim: "#522 cleared — keep the dedicated user.password_reset audit event; brief §3.1.2 amendment + §9 bump ride the merge") | **New audit event: `user.password_reset` (F5).** The Transcorp-staff `/admin/users` surface gains a password-reset action (admin sets a temporary password; the user changes it out-of-band — "force change on next login" is a Phase-1.5 follow-up, no clean Supabase primitive without session middleware). New `resetUserPassword(ctx, { userId, newPassword })` service (gate `user:update`; cross-tenant reset requires `merchant:read_all` per the #259 precedent; `ValidationError` <8 chars; idempotent-free — no mirror write since the credential lives only in `auth.users`). New `resetAuthUserPassword` auth-admin wrapper over `updateUserById({ password })`. §3.1.2 gains `user.password_reset` (body: `email` only; **plaintext password NEVER in the body under any encoding**). Dedicated event kept (Love's ruling) rather than reusing `user.updated` — matches the disable/enable precedent of discrete, forensically-filterable events. No schema delta. RED-first on the permission gate; 9 unit tests. Plan/PR: #522. NOTE: v1.29 assigned next-free against current main at merge-prep (Builder-1's #521 does not touch the brief); additive-only diff verified — the table rows above are untouched (append-only), only this row + the version markers move.
| v1.30 | 21 Jun 2026 (Day 56, move-to-date rework; Love's rulings, verbatim: "build them properly, do not paper over it" + within-schedule "that date already has a delivery — use Skip-without-append to cancel it, or pick a date beyond the schedule") | **Move-to-date builds the moved delivery (§3.1.6).** Move-to-date (`skip` with `target_date_override`) now **cancels the original on SuiteFleet AND materializes a real one-off task at the chosen date** — SF holds exactly ONE delivery. Supersedes the parked Aqib-gated `rescheduleTask` Phase-2 deferral; fixes the gap where the moved task was **never created** (pre-fix net-loss: original live on SF / cancelled with no replacement). `end_date` is NOT extended (no nightly-cron fan-out of intermediate days); the single moved task is created via a new no-cap one-off materializer (`materializeSubscriptionOneOffDate` + `buildOneOffCandidateDatesCte`, single literal date, no `generate_series`, no `LEAST(…, end_date)` cap), run IN the same `withTenant` tx as the skip (atomic — an address gap at the target rolls the whole op back so the original is never cancelled without a replacement). Post-commit: `enqueueCancelTask(original)` for ALL skip variants (the `!isMoveToDate` guard dropped — #535's sound half, corrected) + `enqueueTaskPushBatch([movedTask])` via the standard pipeline. **Within-schedule move-to-date REJECTED at the service layer** — bounded sub `target <= end_date`, open-ended sub (`end_date IS NULL`) any eligible date (every day already delivers; use `skip_without_append`). `subscription.exception.created.outbound_emission` now spans all skip variants incl. move-to-date (`event-types.ts` updated in lock-step). **Timeline move-link (added in the same v1.30 lane):** the move surfaces both directions in the task timeline drawer (§3.3.6) via two new typed per-task audit events `task.moved_in` (new task — "Moved from [old date] / replaces AWB [old AWB]") and `task.moved_out` (cancelled original — "Moved to [new date] / see AWB [new AWB]"), shared correlation_id with `subscription.exception.created`. Persistence shape = audit events (no tasks-table column, **no migration**). The original's AWB is known at move time; the new task's AWB is assigned by SF after the async push, so `getTaskHistory` resolves it at read time from the new task row (AWB-pending until assigned). Four metadata keys added to the R8 allow-list (`moved_from_awb`/`moved_from_delivery_date`/`moved_to_awb`/`moved_to_delivery_date`); counterpart task_ids + correlation_id stay stripped. Drawer gate unchanged (`task:view_timeline`); no new permission. **No schema delta** (the two new audit events are TS-catalogue only — `audit_events.event_type` is plain text). Supersedes parked `#535` (cancel-only; closed as superseded). Filed at `memory/decision_d56_move_to_date_build_properly.md`; design at `memory/handoff_d56_move_to_date_rework.md`. RED-first; unit (service sweep + materializer mapping) + integration (one-off inserts beyond end_date where the range materializer inserts 0). NOTE: v1.30 assigned next-free against main at build time; additive-only — rows above untouched (append-only), only this row + the version markers move.
| v1.31 | 21 Jun 2026 (Day 56, Phase 8 status-distinct-render plan-PR; Love's enumeration rulings, verbatim: "colours group by family … the icon + label make them distinct" + "OUT_FOR_DELIVERY gets the brightest amber" + "in-transit and out-for-delivery are two different statuses") | **SF status renders distinctly — coarse + fine `courier_status` (§3.1.10 + §3.3.11).** A2 mandate (every SF courier state renders distinctly; no collapsing) carried by a new nullable `tasks.courier_status` column (14 fine states) **separate from** the unchanged 8-value `internal_status` coarse lifecycle — render reads fine, falls back to coarse when NULL (Planner-only states + pre-backfill rows). `ARRIVED_ON_DC`/`ARRIVED_IN_DC` fold as one `ARRIVED_AT_DC`; `TASK_HAS_BEEN_UPDATED` stays non-lifecycle. Mappers gain sibling fine maps + a `shouldAdvanceCourierStatus` in-transit ramp guard so the 5 in-transit sub-states progress while coarse stays `IN_TRANSIT`; `DayDisplayStatus` stops folding/mislabeling in-transit vs out-for-delivery. Colours group by family (amber ramp = in-transit with `OUT_FOR_DELIVERY` on hi-vis Signal Amber; red = failure family; Stone-600 = hold family); icon + label disambiguate within a family; **no new hex**. Operators filter by all 14 fine states via a **dropdown** (`?courier_status=` recommended) on **`/tasks` AND the calendar** (OQ-3 Love ruling); `ARRIVED_AT_DC` renders **"Arrived in DC"** (Love's term). `ASSIGNED → Ocean Blue` pill accepted. Blast radius: 9 render surfaces + 3 shared maps + 6 new icons. **Plan-PR only — SQL TO APPLY: no.** Named (not created) migration `0035_tasks_courier_status.sql` parks for Love's SQL authorization at code-PR time; forward-only backfill recommended. #537 (move-to-date timeline drawer) is MERGED — that surface is now normal/coordinated. Filed at `memory/decision_d56_phase8_status_distinct_render.md`; plan `memory/plans/day-56-phase-8-status-distinct-render.md`. NOTE: v1.31 assigned next-free against main at build time; additive-only (rows above untouched, append-only), re-confirm/renumber at merge-prep if a peer bump lands first. |
| v1.32 | 23 Jun 2026 (Day 58, Phase 9 public-surface lane; Love-authorized — landing scope + GCC market-count correction; source mockups #582 login reskin / #583 public marketing landing, Direction B+) | **Two related product decisions: (1) a NET-NEW public marketing landing page is authorized; (2) the GCC market count is corrected 3 → 6.** **(1) Public marketing landing page (net-new scope; amends §1/§2 framing).** Until now the Planner was defined as a Transcorp-owned merchant module entirely BEHIND login — §1 framing plus every §2.1 operator surface (`/`, `/consignees/*`, `/subscriptions/*`, `/tasks`, `/calendar`) is authenticated, and §3.3.9 makes `/` the authenticated merchant-operator landing. Love authorizes a NEW public, UNAUTHENTICATED marketing page that sells Transcorp Planner to meal-plan / subscription merchants. Two entry points only: a **"Log in to Transcorp Planner"** action into the existing authenticated app, and a **"Request access"** CTA. **"Request access" is NOT self-serve signup** — merchants are onboarded by Transcorp Merchant Success; the CTA is a lead-capture / contact path, not an account-creation flow (no public registration is authorized). This is the product's FIRST pre-login surface. **OPEN — `/`-route ownership (recorded open in §8, NOT resolved here):** whether the public landing takes `/` (displacing the §3.3.9 authenticated operator landing to a post-login home / redirect) OR lives on a separate route with `/` staying the authed home — PENDING Love's ruling; the implementation build is blocked on this decision. **Love-gated before public go-live:** final marketing copy and ANY proof claims / partner logos / customer quotes are Love-gated and not authorized for publication until cleared. **(2) Fact correction — GCC market count 3 → 6.** The canonical figure for Transcorp's GCC market footprint is corrected from three to **six markets** (Love-confirmed current reality); the earlier three-market figure (BRD-level) is SUPERSEDED. The public page is authorized to state **"6 markets"** alongside the on-record facts **50,000 packages/day · GCC cold-chain leader** — but those latter two claims are NOT currently asserted anywhere in this brief's body and remain Love-gated marketing claims to be finalized before go-live (only the market count is canonicalized as a corrected fact here). NOTE the three seeded production SuiteFleet regions in §3.1.1 (`transcorp` KSA / `transcorpuae` / `transcorpqatar`, plus `transcorpsb` sandbox) are Planner↔SF integration config — a SEPARATE concern from the business market footprint; this correction does NOT change the seeded region set and rides NO schema/seed delta. **Scope class: docs-only — NO src, NO migration, NO promote.** This entry authorizes the scope and corrects the fact; the landing-page IMPLEMENTATION is a separate forthcoming code build that will reference `PLANNER_PRODUCT_BRIEF.md §1/§2` + this amendment. Filed at `memory/decision_brief_v1_32_amendment_public_landing_and_market_count.md` (companion decision memo lands with this docs PR). NOTE: v1.32 assigned next-free against current main (HEAD `8a69001`); additive-only — every row above is untouched (append-only); only this row plus the version markers (the `**Version:**` line and the closing `**End of …**` line) move; re-confirm / renumber at merge-prep if a peer bump lands first. |
| v1.33 | 23 Jun 2026 (Day 58, Phase 9 public-surface lane; Love's `/`-route ruling, verbatim: "public landing lives at `/welcome`, `/` stays the authenticated operator home … The landing build is no longer route-blocked") | **`/`-route ownership RESOLVED — public marketing landing lives at `/welcome`; `/` stays the authenticated operator home (closes the v1.32 OPEN item).** The v1.32 net-new public landing page (§1/§2 framing; "Log in to Transcorp Planner" + Merchant-Success "Request access") mounts at a NEW dedicated route **`/welcome`**, PUBLIC + unauthenticated. The authenticated merchant-operator landing at `/` (§3.3.9) is UNCHANGED — `/` is NOT displaced; no post-login home move or redirect. The §8 open-questions row is marked RESOLVED to match. **The landing-page implementation build is no longer route-blocked** — it may scaffold against `/welcome` (still gated on final marketing copy + the Love-gated proof claims / logos / quotes per v1.32 before public go-live). `/welcome` is a new public route only — no schema/seed/auth change. **Scope class: docs-only — NO src, NO migration, NO promote** (this entry records the route ruling; the route itself ships in the forthcoming landing code build). Love's ruling recorded inline (no separate decision memo — brief catches up, per the v1.27 idiom). NOTE: v1.33 assigned next-free against current main (HEAD `e01a7b8`, v1.32 merged via #585); additive-only — every row above (incl. v1.32) is untouched (append-only); only this row plus the version markers move; re-confirm / renumber at merge-prep if a peer bump lands first. |
| v1.34 | 23 Jun 2026 (Day 58 / Phase 10, white-dominant surface pass — Batch A0; Love's directive, verbatim: "lift the whole app to WHITE-DOMINANT (Love's standard) … The cream page reads dull vs /welcome's white. Fix CENTRALLY") | **Page field lifted to white-dominant (§3.3.11).** The default page-field value (`--color-surface-primary` / `--color-paper`) is lifted from Snow White cream `#FAF8F4` to a warm near-white **`#FCFBF8`** so WHITE dominates the field; cream recedes to thin framing/borders/section breaks (Ivory `#F2EEE6` / Stone 200 / navy-alpha borders). The value stays *faintly* warm (R>G>B) and uniformly below the Phase-9 B+ card surface `#fffdf8` on R/G so floating cards (`--color-b-card` + the Phase-9-sanctioned `--shadow-b-card` elevation) still read as floating — the field is never stark `#FFFFFF` (the warm card would go dingy). Snow White `#FAF8F4` retained as the warm brand-white / framing reference. **Central change only:** the two surface tokens (hex + `-rgb` companions) in `src/styles/brand-tokens.css` + a white-dominant field wrapper added to BOTH route-group shells (`src/app/(app)/layout.tsx`, `src/app/(admin)/layout.tsx`); NO per-screen edits, NO re-palette of navy/green/amber/red/status/LED, NO migration, NO auth, NO promote. Composition ratio unchanged (white still ~58%). Card-float verified (static harness + real `/welcome`); real `next build` green + typecheck + lint (0 errors) + 2519 unit tests pass. **Blocks the Phase-10 coverage batches** (they rebase on this once it lands). Filed at `memory/decision_phase10_white_dominant_surface.md`. NOTE: v1.34 assigned next-free against current main (HEAD `e9a5d02`, v1.33 merged via #587); additive-only — every row above (incl. v1.33) is untouched (append-only); only this row plus the version markers move; re-confirm / renumber at merge-prep if a peer bump lands first. |
| v1.35 | 25 Jun 2026 (Phase 12.2 Lane 1, materialize capability closure; Love's standing ruling, verbatim: "Transcorp admin should not materialize subscriptions — it is purely Merchant Admin; there should be no other interface for this") | **Cross-tenant materialize closed — `read_all` opens NO action, incl. materialize (§2.3).** §2.3 item 2 already stated the `*_read_all` markers are read-only with "No action capability"; this amendment makes the materialize case EXPLICIT: `subscription:read_all` opens no cross-tenant action, including on-demand task materialization. `triggerManualMaterialization` (`src/modules/subscriptions/service.ts`) is now **same-tenant only** — any cross-tenant trigger is `ForbiddenError` regardless of marker (previously a `subscription:read_all` escape let `transcorp-sysadmin` materialize across tenants, an over-grant against §2.3's stated read-only design). This is **defense-in-depth on the capability**, not just UI: the admin "Materialize" server action is a callable POST endpoint, so removing the button alone (#653) leaves it reachable — only this gate makes it unreachable. The merchant own-tenant path is unbroken (subscription-create `materializeSubscriptionForDateRange`, move-to-date, skip-tail-end, daily cron all bypass this gate). NO permission re-graded: `subscription:read_all` keeps its read uses (`/admin/tasks` · `/admin/consignees` · `/admin/subscriptions`). **Scope class: code-only — one boolean gate + JSDoc + one flipped unit assertion; NO migration, NO DB write, NO auth-config, NO new spend, NO promote.** RED-first (the "allows cross-tenant with `read_all`" case flipped to assert `ForbiddenError`; watched RED→GREEN); 431 unit tests green across subscriptions / subscription-exceptions / task-materialization / cron. Built in **#659**; the `_actions.ts` + `MaterializeButton.tsx` UI removal rides **#653** (D2·A). Love's ruling recorded inline + verbatim as a `LOVE-RULING` comment on #659 (no separate decision memo — brief catches up, per the v1.27/v1.33 idiom; plan at `memory/plans/phase122-materialize-capability-closure.md`, #655). NOTE: v1.35 assigned next-free against current main (HEAD `e01d2ad`, v1.34 merged via #588); additive-only — every row above (incl. v1.34) is untouched (append-only); only this row plus the version markers move; re-confirm / renumber at merge-prep if a peer bump lands first. |
| v1.36 | 25 Jun 2026 (Day 60, Phase 12.2 RELABEL lane; Love's cross-market terminology ruling, verbatim: "'City' is the app-wide label. emirate (in the UAE) = city; the product also runs in KSA, Qatar, etc., where the value is a city — so 'City' is the correct cross-market label") | **User-facing label "Emirate" → "City" app-wide (display-only).** The operator + admin UI retires the UAE-specific noun "Emirate" in favour of the cross-market label **"City"** on every surface: operator `/tasks` column header; consignees list column header; consignee-create form field + validation copy ("Emirate is required." → "City is required."); add-address dialog field; merchant create/edit forms + detail pickup-address label + validation copy ("…when any pickup field is set."); and the two consignee-detail "Emirate / region" labels (operator + admin) for consistency. **Display strings ONLY.** The DB columns (`consignees.emirate_or_region`, `tenants.pickup_address_emirate`, `addresses.emirate`), the internal identifiers (`effectiveEmirate`/`addressEmirate`/`pickupEmirate`, the consignee-cell `emirate` model key), and the **form field NAMEs** (`address_emirate`/`pickup_emirate`/`emirate`) are UNCHANGED — no wire/parse break, **no migration, no auth, no promote**. Brief body form-field LABEL references updated to match (§3.3.1 consignee Address-section list "Emirate"→"City"; §2.1 merchant create/edit field lists "(…, emirate)"→"(…, city)"); the §3.1.1 schema-column reference (`addresses.emirate` text, already noting "or city/country for non-UAE Phase 2") is left intact — it names the column, not a label. Resolves the reviewer-raised LOVE-TRIGGER #2 (brief drift) on the relabel PR. Code ships as #648 (display relabel) + #647 (admin /tasks District + City columns + addresses-join data wiring, T3 — Love-authorized); **this docs amendment merges immediately after #648 so the brief never says "City" while the UI still says "Emirate."** Filed at `memory/decision_brief_v1_36_amendment_emirate_to_city_relabel.md`. NOTE: v1.36 assigned next-free against current main (HEAD `7b8be47`, v1.35 merged via #659); additive-only — every row above (incl. v1.35) is untouched (append-only); only this row plus the version markers move; re-confirm / renumber at merge-prep if a peer bump lands first. |
| v1.37 | 25 Jun 2026 (Day 59 / Phase 12.2 cosmetic pass; Love's directive, verbatim: "#652 (drop Address from operator /tasks + kill scroll) — Love consciously supersedes his Day-53 R6.2/R6.4 ruling … operator /tasks drops Address; District + City suffice") | **Operator `/tasks` table drops the Address column (Day-53 R6.2/R6.4 nine-column ruling SUPERSEDED, in part).** The Day-53 calendar-management diagnostic (`memory/diagnostic_calendar_management_full_surface_enumeration.md` R6.1+R6.2 / R6.4) RULED a nine-column operator `/tasks` layout in which "Address / District / Emirate are three distinct columns matching the schema's three address fields — no flattening into a single cell," with the consignee block (Consignee Name · Address · District · Emirate · Telephone) rendered as one contiguous click target. Love **consciously supersedes** that ruling for the Address column only: the operator `/tasks` table **drops the Address (street/building, `consignees.addressLine`) column** — **District + City suffice** ("City" being the post-relabel header over the Emirate value per the companion Emirate→City relabel lane). The horizontal scroll is removed in the same change (the forced min table-width is dropped). This is a **VIEW change only** — `consignees.addressLine` and the override fall-through are still computed and still drive routing/labels; the column is simply no longer rendered. The remaining contiguous click target spans Consignee · District · City · Telephone. **No data path, no query, no permission, no migration, no cost.** The Day-53 diagnostic record stays UNTOUCHED in its own file (append-only); this entry retires that one point going forward. Companion lane: the Emirate→City relabel (#648). Held to merge ALONGSIDE the code change (#652). Love's ruling recorded inline here + as a LOVE-RULING comment on #652 (no separate decision memo — brief catches up, per the v1.27 idiom). NOTE: v1.37 assigned next-free against current main (HEAD `45c73a3`, v1.36 merged via #649); additive-only — every row above (incl. v1.36) is untouched (append-only); only this row plus the version markers move; re-confirm / renumber at merge-prep if a peer bump lands first. |

### Day-51 amendment — auto-resume cron schedule degradation (operational)

Auto-resume cron schedule changed from every 15 minutes (`*/15 * * * *`) to daily at 11:00 UTC (`0 11 * * *`). Driver: Vercel platform plan downgrade from Pro to Hobby tier on or around 2026-06-04. Hobby tier permits only daily cron schedules.

Product impact: paused subscriptions auto-resume with up to 24-hour latency relative to their stored resume-date, vs the prior up-to-15-minute latency. The 11:00 UTC schedule fires one hour before the daily materializer (`0 12 * * *`), so a subscription whose pause-end-date crosses on day D will be resumed at 11:00 UTC day D, then materialized at 12:00 UTC day D. No delivery-day miss for D-day pause-end dates.

Edge case: if an operator sets a pause-end-date such that the resume must take effect before 11:00 UTC on day D (e.g., 06:00 UTC delivery window), the auto-resume runs in time at 11:00 UTC of the prior day's materializer pull won't have caught it. Mitigation: this is rare in practice (delivery windows in MPL/Dubai operational hours don't go below 09:00 Dubai = 05:00 UTC; auto-resume runs at 11:00 UTC well after the early delivery window). If it surfaces in production, an operator can resume the subscription manually via the existing operator action.
