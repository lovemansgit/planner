...
- `subscription.resume_reactivations_pushed` (v1.22) — body includes `subscription_id`, `correlation_id`, `actual_resume_date`, `reactivated_task_count`, `enqueued_count`, `failed_chunks`, `previous_awbs`. Outbound leg of resume re-sync (R16, plan #408); paired with `subscription.resumed` via the pause window's `correlation_id`. Emitted only when ≥1 restored task carried an SF AWB (re-created fresh on SF — cancel is terminal vendor-side); partial fan-out failure emits-then-re-throws (R2 posture).
