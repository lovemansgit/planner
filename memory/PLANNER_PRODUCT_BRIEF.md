...
10. For address overrides: update affected tasks' `address_id` per scope (v1.18, R4/R5 rulings): one-off = the single task at the date; forward = every in-horizon task on THIS subscription (`delivery_date >= start_date AND < CURRENT_DATE + 14 days`; sibling subscriptions untouched — subscription-scoped, NOT consignee-scoped). Pushed rows flip `outbound_sync_state='pending_update'` (migration 0029)
10b. (v1.18) Post-commit SF outbound push for the override: one-off enqueues a single `task-resource:update` via `enqueueUpdateTask`; forward fans out via `enqueueBulkUpdateTasks` (one message per pushed in-horizon task). The wire patch carries a SERVER-BUILT ConsigneeSnapshot read from the override `addresses` row + `consignees` row (option B per the Day-52 ruling on `memory/followup_address_edit_sf_outbound_gap.md`). >14-day-out future tasks need no push — the exception row is read by the materializer CTE forward branch and they materialize at the new address
