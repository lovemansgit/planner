...
Per-handler-invocation log (one log line per push attempt, structured): `{ tenant_id, task_id, sf_latency_ms, outcome }` where `outcome ∈ {tenant_mismatch_rejected, address_id_null_rejected, task_already_pushed_pre_check, success, awb_exists_reconciled, awb_reconcile_failed_retry_throw, failed_to_dlq, skipped_district, tenant_skipped_no_credentials, task_already_pushed_in_push, task_not_found}` (11 values per the canonical TypeScript union at [`src/app/api/queue/push-task/route.ts:72-83`](../../src/app/api/queue/push-task/route.ts#L72-L83); replaces this section's earlier 5-value sketch — actual implementation has more distinct states across pre-pushSingleTask guards, the 8-kind SinglePushOutcome adapter result, and the defensive task-not-found branch). Aggregates derive from this log line.
...
| **Address_override_forward — most-recent-wins supersession (D7-1 tighten):** sub D has forward override A starting Wed week 1 → tasks Wed week 1 onward use override A via Layer 2. Operator creates second forward override B starting Fri week 1 → Layer 2's `ORDER BY e.start_date DESC LIMIT 1` selects override B for Fri week 1 onward; Wed/Thu week 1 continue resolving to override A (their `start_date <= task.target_date` filter still includes A but excludes B). Pattern: most-recent applicable forward override wins; earlier overrides aren't deleted, just superseded by date-bounded scope. | §2.3 Layer 2 ORDER BY DESC / §2.4 row 4 |
...
| **Signature gate:** missing or invalid QStash signature → 403 returned; no body parsing | §5.1 `verifySignatureAppRouter` |
| **Observability log shape (NEW, §5.5):** trigger handler under each `outcome` enum value; assert log line emitted with `{ tenant_id, task_id, sf_latency_ms: <number>, outcome: <one-of-11-states> }`. Outcome enum strict-check (no string drift), per the canonical TypeScript union at [`src/app/api/queue/push-task/route.ts:72-83`](../../src/app/api/queue/push-task/route.ts#L72-L83): `tenant_mismatch_rejected` / `address_id_null_rejected` / `task_already_pushed_pre_check` / `success` / `awb_exists_reconciled` / `awb_reconcile_failed_retry_throw` / `failed_to_dlq` / `skipped_district` / `tenant_skipped_no_credentials` / `task_already_pushed_in_push` / `task_not_found` | §5.5 observability surface |
...
| **failureCallback handler — signature gate:** unsigned POST to `/api/queue/push-task-failed` → 403; no DB write | §5.2 endpoint security |
