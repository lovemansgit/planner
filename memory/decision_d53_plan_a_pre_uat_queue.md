...
- **Scaling-rules memo** — referenced by Love's ruling ("three builder pairs run
  in parallel under the scaling rules memo") but **not yet filed in the repo** at
  the time of writing. The three-pair parallel orchestration is set up under that
  memo; this record does not fabricate its path. Flag to fill the cross-ref once
  the scaling-rules memo lands.
