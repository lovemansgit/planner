...
- **Should see (these four older orders):** a "Proof of delivery" panel opens with a **styled "Photo expired at the delivery vendor" image** (navy-on-ivory empty state with the camera glyph and the 7-day explanation line — H3, Day-53 EVE). That's **correct and expected** — these photos were delivered in May, before Planner captured photos durably, and SuiteFleet's links die after 7 days. The point being demonstrated: Planner shows the operator an **honest, branded empty state** (no broken-image glyph, no vendor URL). _(Supersedes the earlier broken-image-placeholder line — the 410-with-broken-image observable was replaced by this styled image in the Day-53 EVE durable-POD lane, Love-ruled.)_
- **For a LIVE photo (do this opportunistically during UAT):** the first time a driver completes a **fresh** sandbox delivery **with** a photo upload, find that order and open its POD — the actual photo renders in the panel, and from Day-53 EVE onward Planner also **captures a durable copy within minutes of delivery** (the photo keeps rendering after SuiteFleet's 7-day link expiry). There was no within-7-day delivered-with-photo order to demo at sheet-writing time (see §POD-RESULT).
